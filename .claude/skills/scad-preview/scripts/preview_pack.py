...
def main() -> None:
    ...
    stl = Path(a.stl) if a.stl else next(iter(sorted(out.glob("*.stl"))), None)
    ...
    brut = stl_binary(stl)
    mo = len(brut) / 1e6
    ascii_mo = stl.stat().st_size / 1e6
    if mo > SEUIL_MO:
        sys.exit(
            f"{stl.name} pese {mo:.1f} Mo en binaire (seuil {SEUIL_MO} Mo). En base64 il "
            f"occuperait {mo * 1.33:.1f} Mo de la page. Regenere-le plus grossierement "
    ...
    print(f"STL     : {stl.name} — {ascii_mo:.2f} Mo ASCII -> {mo:.2f} Mo binaire "
